...
public class MyPanel extends JPanel {
	...
	public MyPanel() {   //This is the constructor... this code runs first to initialize
		...
		for(int x = 0; x < 9; x++) {	//Sets random mines and stores coordinates
			int h = generator.nextInt(8) + 1;
			int w = generator.nextInt(8) + 1;
			colorArray[h][w] = hiddenMineColor;
		}
	...
	public void revealAdjacentTiles(int x, int y) {
		if((x<0) || (y<0) || (x>=9) || (y>=9)){
			...
			if(!(colorArray[x][y].equals(hiddenMineColor))) {
			colorArray[x][y] = Color.WHITE;
			revealAdjacentTiles(x-1, y);
			revealAdjacentTiles(x+1, y);
			revealAdjacentTiles(x, y-1);
			revealAdjacentTiles(x, y+1);
		}else {
			return;
		}
